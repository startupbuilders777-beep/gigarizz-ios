...
final class RateLimiter: ObservableObject {
    ...
    func checkPhotoGeneration(isPro: Bool) -> (allowed: Bool, message: String?) {
        ...
        if used >= limit {
            if isPro {
                return (false, "You've used all \(limit) pro generations today. Resets at midnight.")
            } else {
                return (false, "Free tier limit reached (\(limit)/day). Upgrade to Pro for \(AIAPIConfig.proGenerationsPerDay) generations/day!")
            }
        }
