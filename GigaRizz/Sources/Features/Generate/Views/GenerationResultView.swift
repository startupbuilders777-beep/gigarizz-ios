...
struct GenerationResultView: View {
    ...
    // Demo images for testing (in production, these would come from API)
    private var demoImages: [UIImage] {
        // Create gradient placeholder images for demo
        generatedPhotos.compactMap { _ in
            createGradientImage()
        }
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .overlay {
                ...
                if photoLibraryService.isSaving {
                    savingProgressOverlay
                }

                // Success confirmation
                if photoLibraryService.showSuccessConfirmation {
                    successOverlay
                }
    ...
    private var actionButtons: some View {
        VStack(spacing: DesignSystem.Spacing.s) {
            ...
            GRButton(
                ...
                isLoading: photoLibraryService.isSaving
            ) {
                Task { await saveAllPhotos() }
            }
    ...
    private func saveAllPhotos() async {
        do {
            let images = demoImages
            let metadata = PhotoLibraryService.metadataForGeneratedPhoto(
                style: style,
                userId: userId
            )
            _ = try await photoLibraryService.saveImages(images, metadata: metadata)
    ...
    private func sharePhotos() {
        ...
        // In production, would share actual images
        // For demo, just show UIActivityViewController
    }
    ...
    private var savingProgressOverlay: some View {
        VStack {
            Spacer()
            SaveProgressView(
                savedCount: photoLibraryService.savedCount,
                totalToSave: photoLibraryService.totalToSave
            )
            .padding(.bottom, DesignSystem.Spacing.xxl)
        }
        .transition(.opacity.combined(with: .scale))
        .animation(DesignSystem.Animation.smoothSpring, value: photoLibraryService.isSaving)
    }
    ...
    private var successOverlay: some View {
        VStack {
            Spacer()
            SaveSuccessView(savedCount: photoLibraryService.savedCount)
                .padding(.bottom, DesignSystem.Spacing.xxl)
        }
        .transition(.move(edge: .bottom).combined(with: .opacity))
        .animation(DesignSystem.Animation.smoothSpring, value: photoLibraryService.showSuccessConfirmation)
    }
    ...
    private var permissionDeniedOverlay: some View {
        ...
        .transition(.opacity)
        .animation(DesignSystem.Animation.smoothSpring, value: showPermissionDenied)
    }

    // MARK: - Demo Image Creation
    ...
    private func createGradientImage() -> UIImage? {
        ...
        return renderer.image { context in
            let colors: [UIColor] = [
                UIColor(DesignSystem.Colors.flameOrange),
                UIColor(DesignSystem.Colors.goldAccent)
            ]
